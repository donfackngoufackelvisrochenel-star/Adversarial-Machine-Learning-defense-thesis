% ===========================================================================
%  MEng Dissertation — Adversarial Machine Learning Defense on IoMT Data
...
\begin{center}
    ...
    {\Huge\bfseries Implementation and Evaluation of Defense Strategies Against Evasion Attacks in Adversarial Machine Learning on IoMT Data}\\[1.0cm]
...
I, \textbf{MALWARE XXXXXXXX XXXXXXXXX}, Registration Number: \textbf{XX-XXXX-XX}, in the Department of \textbf{Computer Engineering}, National Higher Polytechnic Institute, The University of Bamenda hereby declare that this work titled \textbf{``Implementation and Evaluation of Defense Strategies Against Evasion Attacks in Adversarial Machine Learning on IoMT Data''} is my original work. It has not been presented in any application for a degree or any academic pursuit elsewhere. I have acknowledged all borrowed ideas nationally and internationally through citations.
...
Having met the stipulated requirements, this dissertation entitled \textbf{``Implementation and Evaluation of Defense Strategies Against Evasion Attacks in Adversarial Machine Learning on IoMT Data''} has been accepted by the Postgraduate School of The University of Bamenda in partial fulfillment of the requirements for the award of a \textbf{Master of Engineering (MEng) Degree in Computer Engineering} (Option: Software Engineering).
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering [Screenshot of correlation heatmap from dashboard --- Insert Here]}}
...
\begin{table}[H]
    \centering
    \caption{Model hyperparameters used in experiments}\label{tab:hyperparams}
    \begin{tabular}{lccc}
        \toprule
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering [Screenshot of trained models dashboard --- Insert Here]}}
...
\begin{table}[H]
    \centering
    \caption{Clean performance of base classifiers on CIC\_IoMT\_2024}\label{tab:clean_performance}
    \begin{tabular}{lcccc}
        \toprule
...
\begin{table}[H]
    \centering
    \caption{Classifier accuracy under FGSM attack at varying epsilon values}\label{tab:fgsm_impact}
...
\begin{table}[H]
    \centering
    \caption{Classifier accuracy under PGD attack at varying epsilon values}\label{tab:pgd_impact}
    \begin{tabular}{lcccccc}
        \toprule
        Model & $\epsilon=0$ & $\epsilon=0.01$ & $\epsilon=0.05$ & $\epsilon=0.1$ & $\epsilon=0.2$ & $\epsilon=0.5$ \\
        \midrule
...
\begin{table}[H]
    \centering
    \caption{Accuracy of defense strategies under FGSM attack}\label{tab:defense_fgsm}
    \begin{tabular}{lcccccc}
        \toprule
        ...
        Ensemble (soft vote) & 0.8495 & 0.0827 & 0.0727 & 0.0585 & 0.0508 & 0.0363 \\
        Feature Selection    & 0.1634 & 0.1516 & 0.1515 & 0.0947 & 0.0998 & 0.0775 \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Adversarial training} provided the most balanced robustness. While it reduced clean accuracy to 61.34\% (a 32.36 percentage-point drop from undefended XGBoost), it maintained the highest accuracy under attack across all epsilon values, peaking at 38.03\% at $\epsilon = 0.1$. This demonstrates that adversarially-trained models sacrifice some clean performance but gain substantial robustness.

\textbf{The ensemble defense} preserved the highest clean accuracy among defended models (84.95\%), but its performance collapsed under even small perturbations (8.27\% at $\epsilon = 0.01$). This suggests that while the ensemble is a strong classifier on clean data, its robustness does not exceed the weakest link since the FGSM attack perturbs all three models simultaneously.
...
\begin{table}[H]
    \centering
    \caption{Accuracy of defense strategies under PGD attack}\label{tab:defense_pgd}
    \begin{tabular}{lcccccc}
        \toprule
        Defense & $\epsilon=0$ & $\epsilon=0.01$ & $\epsilon=0.05$ & $\epsilon=0.1$ & $\epsilon=0.2$ & $\epsilon=0.5$ \\
        \midrule
        None (XGBoost)       & 0.9370 & 0.1242 & 0.1696 & 0.1718 & 0.1675 & 0.1535 \\
        Adversarial Training & 0.6134 & 0.2261 & 0.2584 & 0.2718 & 0.2536 & 0.2134 \\
        Ensemble (soft vote) & 0.8495 & 0.0854 & 0.0804 & 0.0746 & 0.0604 & 0.0480 \\
        Feature Selection    & 0.1634 & 0.1444 & 0.1455 & 0.1291 & 0.1147 & 0.0945 \\
        \bottomrule
    \end{tabular}
\end{table}

The results under PGD follow a similar pattern to FGSM, with adversarial training again providing the best robustness. The PGD attack is slightly more effective against undefended models at $\epsilon = 0.01$ (12.42\% vs.~18.38\% for FGSM), but the relative ranking of defenses remains consistent.
...
\begin{lstlisting}[language=Python, basicstyle=\small\ttfamily, breaklines=true, frame=single, caption=Data loading pipeline (loader.py)]
def load_data(filepath=None, chunksize=None, max_rows=None):
    if filepath is None:
    ...
    if filepath.is_dir():
        ...
        csv_files = list(filepath.rglob("*.csv"))
        if parquet_files:
    ...
    chunks = []
    rows_loaded = 0
    for chunk in pd.read_csv(filepath, header=header, names=names,
...
\begin{lstlisting}[language=Python, basicstyle=\small\ttfamily, breaklines=true, frame=single, caption=FGSM attack for tree-based models (fgsm.py)]
def fgsm_attack(model, X, y, epsilon=0.1, feature_bounds=None):
    ...
    for i in range(len(X_adv)):
        if preds[i] == y[i]:
            ...
            gradient_sign = -np.sign(probs[i, preds[i]] - probs[i, y[i]]) * imp
        X_adv[i] += eps * gradient_sign
...
\begin{lstlisting}[language=Python, basicstyle=\small\ttfamily, breaklines=true, frame=single, caption=Adversarial training defense (adversarial\_training.py)]
def adversarial_training(model_name, X_train, y_train,
                         X_val=None, y_val=None, epsilon=0.1, epochs=1):
    ...
    for epoch in range(epochs):
        ...
        X_adv_pool = pd.concat([X_adv_pool, X_adv], axis=0)
        y_adv_pool = pd.concat([y_adv_pool, y_train], axis=0)
...
\section{Confusion Matrix for XGBoost (Clean Data)}
...
\begin{table}[H]
    \centering
    \caption{XGBoost confusion metrics (top 10 classes by FN)}\label{tab:confusion_xgb}
    \begin{tabular}{lrrrrrr}
        \toprule
        Class & TP & TN & FP & FN & Prec & Rec \\
        \midrule
        ... & ... & ... & ... & ... & ... & ... \\
        \bottomrule
    \end{tabular}
...
\section{Robustness Comparison Plots}

Figure~\ref{fig:robustness_fgsm} and Figure~\ref{fig:robustness_pgd} show the robustness curves for all defense strategies under FGSM and PGD attacks respectively.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering [Insert robustness\_FGSM.png from reports/ --- Insert Here]}}
    \caption{Robustness comparison under FGSM attack}\label{fig:robustness_fgsm}
\end{figure}

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering [Insert robustness\_PGD.png from reports/ --- Insert Here]}}
    \caption{Robustness comparison under PGD attack}\label{fig:robustness_pgd}
\end{figure}
